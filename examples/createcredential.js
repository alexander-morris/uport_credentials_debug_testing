...
app.get('/', (req, res) => {
  credentials.createDisclosureRequest({
    ...
    messageLogger('requestToken', requestToken, "-+0-")
...
app.post('/callback', (req, res) => {
  ...
  messageLogger('jwt', jwt)
  credentials.authenticateDisclosureResponse(jwt).then(creds => {
    messageLogger('creds', creds)
    const did = creds.did
    ...
    credentials.createVerification({
      ...
      messageLogger(att, 'Encoded Attestation Sent to User (Signed JWT)', "-+1-")
      messageLogger(decodeJWT(att), 'Decoded Attestation Payload of Above', "-+2-")
      return push(att)
    }).then(res => {
      messageLogger('Push notification with attestation sent, will recieve on client in a moment', "-+3-")
      ngrok.disconnect()
    })
  })
})
...
const server = app.listen(8088, () => {
  ngrok.connect(8088).then(ngrokUrl => {
    endpoint = ngrokUrl
    console.log(`Attestation Creator Service running, open at ${endpoint}`, "-+4-")
  });
})
